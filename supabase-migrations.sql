...
-- ═══ 31. BBO Extended Fields ═══
-- Additional broker-only fields for showing prep, commission, occupant info.
-- These are RESO Data Dictionary fields synced from Navica but only visible to admin.
...
-- ═══════════════════════════════════════════════════════════════
-- CMA (Comparative Market Analysis) — Mountain-Specific Tool
-- AI-powered feature extraction, paired sales, adjustment engine
-- ═══════════════════════════════════════════════════════════════

-- ═══ 39. CMA Feature Tags — AI-extracted mountain features per listing ═══
CREATE TABLE IF NOT EXISTS cma_feature_tags (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  listing_key TEXT NOT NULL,
  agent_id UUID,  -- NULL = site owner, UUID = future SaaS agent

  -- Mountain-specific feature ratings (1-5 scale)
...
-- ═══ 40. CMA Reports — Each CMA report instance ═══
...
-- ═══ 41. CMA Adjustments — Per-comp, per-feature adjustment records ═══
CREATE TABLE IF NOT EXISTS cma_adjustments (
  ...
  -- Comp snapshot at report creation
  comp_data JSONB DEFAULT '{}',
  comp_features JSONB DEFAULT '{}',

  -- Standard adjustments (dollar amounts)
  ...
  -- Slider states (-2 to +2: much worse to much better)
