...
function cmaRenderStep4() {
  ...
  html += '<div id="cmaSubjectPhotoWrap">' + cmaSubjectPhotosInner() + '</div>';
  html += '</div>';
  html += '</div>';
...
// ── Subject photos (first = report cover/hero; the rest fill the photo grid page) ──
function cmaGetSubjectPhotos() {
  var l = _cmaState.subject && _cmaState.subject.listing;
  if (!l) return [];
  if (!Array.isArray(l.uploaded_photos)) {
    l.uploaded_photos = l.uploaded_photo_url ? [l.uploaded_photo_url] : []; // seed from legacy single field
  }
  return l.uploaded_photos;
}

function cmaSyncSubjectPhotoLegacy() {
  var l = _cmaState.subject && _cmaState.subject.listing;
  if (!l) return;
  var arr = cmaGetSubjectPhotos();
  if (arr.length) { l.uploaded_photo_url = arr[0]; } else { delete l.uploaded_photo_url; }
}

function cmaSubjectPhotosInner() {
  var photos = cmaGetSubjectPhotos();
  var input = '<input type="file" id="cmaSubjectPhotoInput" accept="image/jpeg,image/png,image/webp" multiple style="display:none" onchange="cmaUploadSubjectPhoto(this.files)" />';
  var html = '';
  if (photos.length) {
    html += '<div style="display:flex;flex-wrap:wrap;gap:0.6rem;margin-bottom:0.6rem">';
    photos.forEach(function(u, i) {
      var border = i === 0 ? '2px solid var(--gold,#8B7748)' : '1px solid #ddd';
      html += '<div style="position:relative;width:150px">';
      html += '<img src="' + esc(u) + '" alt="Subject photo ' + (i + 1) + '" style="width:150px;height:100px;border-radius:6px;object-fit:cover;display:block;border:' + border + '" />';
      if (i === 0) html += '<span style="position:absolute;top:4px;left:4px;background:rgba(0,0,0,0.65);color:#fff;font-size:0.6rem;padding:1px 6px;border-radius:3px">Cover</span>';
      html += '<button class="crm-btn crm-btn-secondary" type="button" title="Remove" onclick="cmaRemoveSubjectPhoto(' + i + ')" style="position:absolute;top:2px;right:2px;padding:0 7px;font-size:0.9rem;line-height:1.5">&times;</button>';
      if (i !== 0) html += '<button class="crm-btn crm-btn-secondary" type="button" onclick="cmaMakeSubjectCover(' + i + ')" style="position:absolute;bottom:3px;left:3px;padding:1px 6px;font-size:0.6rem">Make cover</button>';
      html += '</div>';
    });
    html += '</div>';
  }
  html += '<button class="crm-btn crm-btn-secondary" type="button" onclick="document.getElementById(\'cmaSubjectPhotoInput\').click()">' + (photos.length ? 'Add more photos' : 'Upload photo(s)') + '</button>';
  html += '<span style="font-size:0.75rem;color:#888;margin-left:0.6rem">JPG or PNG. First photo is the report cover; the rest appear on a photo page.</span>';
  return html + input;
}

function cmaRefreshSubjectPhotos() {
  var wrap = document.getElementById('cmaSubjectPhotoWrap');
  if (wrap) wrap.innerHTML = cmaSubjectPhotosInner();
}
...
async function cmaUploadSubjectPhoto(files) {
  ...
  var arr = cmaGetSubjectPhotos();
  toast('Uploading ' + list.length + ' photo' + (list.length > 1 ? 's' : '') + '...', 'info');
  var ok = 0;
  for (var k = 0; k < list.length; k++) {
    var file = list[k];
    if (!/^image\//.test(file.type)) { toast('Skipped a non-image file', 'error'); continue; }
    if (file.size > 10 * 1024 * 1024) { toast('Skipped an image over 10MB', 'error'); continue; }
    try {
      var ext = ((file.name.split('.').pop() || 'jpg').toLowerCase().replace(/[^a-z0-9]/g, '')) || 'jpg';
      var base = _cmaState.subject.listing.listing_key || _cmaState.reportId || 'subject';
      var path = 'subject/' + base + '-' + Date.now() + '-' + k + '.' + ext;
      var up = await _sb.storage.from('cma-photos').upload(path, file, { upsert: true, contentType: file.type });
      if (up.error) { toast('Upload failed: ' + up.error.message, 'error'); continue; }
      arr.push(_sb.storage.from('cma-photos').getPublicUrl(path).data.publicUrl);
      ok++;
    } catch (e) { console.error('Subject photo upload error', e); toast('Upload error', 'error'); }
  }
  cmaSyncSubjectPhotoLegacy();
  cmaRefreshSubjectPhotos();
  if (ok) toast(ok + ' photo' + (ok > 1 ? 's' : '') + ' added', 'success');
}

function cmaRemoveSubjectPhoto(idx) {
  var arr = cmaGetSubjectPhotos();
...
function cmaMakeSubjectCover(idx) {
  var arr = cmaGetSubjectPhotos();
  if (idx > 0 && idx < arr.length) arr.unshift(arr.splice(idx, 1)[0]);
  cmaSyncSubjectPhotoLegacy();
  cmaRefreshSubjectPhotos();
}
