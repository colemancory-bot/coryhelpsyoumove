...
function generatePosts(listing: ListingData) {
  ...
  // SEO-friendly URL with address slug
  const slug = `${address} ${city} nc`.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  const propUrl = `https://coryhelpsyoumove.com/?listing=${slug}`;
