...
- ✅ **A done — 10 landing pages live.** Original 7 (`mountain-cabins-bryson-city-nc` reference, `homes-for-sale-waynesville-nc-under-400k`, `cabins-for-sale-maggie-valley-nc`, `unrestricted-land-for-sale-franklin-nc`, `homes-for-sale-sylva-nc-under-300k`, `homes-for-sale-cashiers-nc-under-500k`, `waynesville-nc-homes-with-acreage`) plus 3 more (`jackson-county-nc-land-for-sale`, `cullowhee-nc-rental-investment-property`, `gated-communities-cashiers-nc`). Each is unique and content-rich (700-965 words), Fair-Housing-compliant, no em dashes, with CollectionPage/BreadcrumbList/FAQPage schema, a verified pre-filtered search CTA (type, price-band, multi-location, all confirmed), and a sitemap entry. "Bryson City log cabins" was dropped to avoid duplicating the cabins reference page; Macon County land is folded into the Franklin page. Next candidates if you want more: a dedicated Macon County land page, Maggie Valley investment property, creek-front property, log-cabin builders.
- 🔎 **MLS statement-timeout diagnosis.** Likely an RLS-vs-index predicate mismatch: the compliance support index (`idx_mls_listings_compliance_gate`) uses strict `internet_entire_listing_display_yn = TRUE`, but the RLS policy uses `COALESCE(...,TRUE)`, so the planner can't use the selective index, falls back to a broad scan over all `mlg_can_view` rows, and intermittently exceeds the 8s anon `statement_timeout` under sync write-load. Fix written as migration `20260602000001` (partial index matching the RLS `COALESCE` form + `ANALYZE`); apply after `EXPLAIN ANALYZE` verification on the live DB. The D fallback covers the user-facing symptom meanwhile. The secondary loser-attribution bug (siblings query returns zero rows for anon under the winner-only RLS, so cross-MLS attribution doesn't populate) is fixed by migration `20260602000002` (RLS allowing anon to read IDX-eligible active losers for attribution). **Both migrations are NOT auto-applied** — they touch a compliance-critical table and need DB-side verification first.
