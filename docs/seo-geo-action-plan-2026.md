...
- ✅ **A done — 7 landing pages live.** `mountain-cabins-bryson-city-nc.html` (reference) plus `homes-for-sale-waynesville-nc-under-400k`, `cabins-for-sale-maggie-valley-nc`, `unrestricted-land-for-sale-franklin-nc`, `homes-for-sale-sylva-nc-under-300k`, `homes-for-sale-cashiers-nc-under-500k`, `waynesville-nc-homes-with-acreage`. Each is unique and content-rich (770-965 words), Fair-Housing-compliant, no em dashes, with CollectionPage/BreadcrumbList/FAQPage schema, a pre-filtered search CTA (type or price-band, both verified), and a sitemap entry. "Bryson City log cabins" was dropped to avoid duplicating the cabins reference page. Next candidates if you want more: Jackson/Macon county land, Cullowhee/WCU rental investment, gated communities Cashiers, Maggie Valley investment property.
- 🔎 **MLS statement-timeout diagnosis.** Likely an RLS-vs-index predicate mismatch: the compliance support index (`idx_mls_listings_compliance_gate`) uses strict `internet_entire_listing_display_yn = TRUE`, but the RLS policy uses `COALESCE(...,TRUE)`, so the planner can't use the selective index, falls back to a broad scan over all `mlg_can_view` rows, and intermittently exceeds the 8s anon `statement_timeout` under sync write-load. Proposed fix: a partial index matching the RLS `COALESCE` form, plus `ANALYZE`, verified with `EXPLAIN ANALYZE` before applying. Not applied (compliance-critical table). The D fallback already covers the user-facing symptom. Secondary: the loser-attribution siblings query (`is_winner=false`) now returns zero rows for anon under the same RLS policy, so cross-MLS attribution silently doesn't populate publicly.
